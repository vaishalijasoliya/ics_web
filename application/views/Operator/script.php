...
<script type="text/javascript">
    ...
    function load_meter_connections() {
        ...
        if (response.status != false) {
            ...
            response.forEach(element => {
                rows += '<tr><td>' + element.serial_number + '</td><td>' + element.meter_name + '</td><td>' + element.channel_name + '</td><td>' + element.property + '</td><td>' + element.meter_type + '</td><td><a href="#"  data-toggle="modal" data-target="#edit-details" data-id="' + element.id + '" data-image="' + element.image + '"  data-wr_number="' + element.wr_number + '" data-meter_name="' + element.meter_name + '" data-serial_number="' + element.serial_number + '"data-property="' + element.property + '"data-telementry="' + element.telementry + '" data-channel_id="' + element.channel_id + '"  data-type_id="' + element.type_id + '"data-f_rate_name="' + element.f_rate_name + '"data-f_rate_scal="' + element.f_rate_scal + '"data-f_total_name="' + element.f_total_name + '"data-f_total_scal="' + element.f_total_scal + '"><img src="https://portal.intelligentcanalsystems.com/assets/operators/img/edit-meter.png" alt="meter"></a><a href="#"  data-toggle="modal" data-target="#myModal" data-id="' + element.id + '" data-name="' + element.meter_name + '"><img src="https://portal.intelligentcanalsystems.com/assets/operators/img/delete-icon.png" alt="delete-icon"></a></td></tr>'
            });
    ...
    var rule1 = $("#meter_connection_form").validate({
        rules: {
            meter_name: {
                required: true,
            },
        ...
        messages: {
            ...
            flow_rate_topic_name: {
                required: "Please enter Flow Rate Topic Name"
            },
            flow_rate_scaling: {
                required: "Please enter Flow Rate Scaling",
                number: "Please enter Only Number",
                min: "Please enter Minimum Length of 0.0001",
                max: "Please enter Maximum Length of 9999"
            },
            flow_total_reading: {
                required: "Please enter Flow Total Reading Topic Name"
            },
            flow_total_reading_scaling: {
                required: "Please enter Flow Total Reading Scaling",
                number: "Please enter Only Number",
                min: "Please enter Minimum Length of 0.0001",
                max: "Please enter Maximum Length of 9999"
            }
        ...
        submitHandler: function(form) {
            ...
            if ($(form).find('input[name="userid"]').val() === '') {
                ...
                var ck = $(form).find('input[name="Telemetry"]').is(":checked") ? ck = '1' : ck = '0';

                var flow_rate_topic_name = $(form).find('input[name="flow_rate_topic_name"]').val()
                var flow_rate_scaling = $(form).find('input[name="flow_rate_scaling"]').val()

                var flow_total_reading = $(form).find('input[name="flow_total_reading"]').val()
                var flow_total_reading_scaling = $(form).find('input[name="flow_total_reading_scaling"]').val()
                ...
                // }
                // else if ($(form).find('input[name="Telemetry"]').is("checked")) {
                //     flow_total_reading.val()
                //     flow_total_reading_scaling.required
                // } else if ($(form).find('input[name="Telemetry"]').is("checked") == true) {
                //     flow_rate_topic_name.val() == ''
                //     flow_rate_scaling.required
                // } else if ($(form).find('input[name="Telemetry"]').is("checked") == true) {
                //     flow_total_reading.val() == ''
                //     flow_total_reading_scaling.required
                // } else {
                ...
                fd.append('Telemetry', ck);
                fd.append('flow_rate_topic_name', flow_rate_topic_name);
                fd.append('flow_rate_scaling', flow_rate_scaling);
                fd.append('flow_total_reading', flow_total_reading);
                fd.append('flow_total_reading_scaling', flow_total_reading_scaling);
    ...
    function load_live_flow() {
        ...
        request.done(function(msg) {
            ...
            console.log('load live flow');
            $("#ud_clientdetail_username").html(username);
            $("#tr_clientdetail_username").html(username);
            $(".load_client_username").html(username);
            $("#lw_username").html(username);
            $("#tr_username").html(username);
            $("#ud_clientdetail_contactname").html(contactname);
        });
        request.fail(function(jqXHR, textStatus) {
    ...
    $('#ud_username, #channel,#all_meters').change(function() {
        console.log('floData');
        load_live_flow_data();
    }); 
    function load_live_flow_data() {
        var userid = $('#ud_username').val();
        var channel = $('#channel').val();
        var allmeter = $('#all_meters').is(':checked')?1:0;

        var individual_meter = $('#individual_meter').val();
        
        var origin = "<?php echo base_url(); ?>";
        //var origin = "<?php echo base_url(); ?>";
        var siteurl = origin + '/operator/load_live_flow_meters';
        var data = {
            userid: userid,
            channel: channel,
            // todate: todate,
        ...
        request.done(function(msg) {
            ...
            data.forEach(element => {
                ...
                Total_Usage += Number(element.reading);
                Remaining += Number(element.reading);
                Allocation += Number(element.wr_volume);
            });
            rows += '<tr style="border-top: 2px solid #ddd;"><td style="font-weight:bold">Total:</td><td></td><td></td><td></td><td></td><td style="font-weight:bold">' + Total_Usage + ' ML</td><td style="font-weight:bold">' + Remaining + ' ML</td><td style="font-weight:bold">' + '' + '</td><td></td></tr>';

            $('#live_flow > tbody').html(rows);
            btnGetCount()
    ...
    // live flow change user  
    $('#lf_username').change(function() {
        var userid = $('#lf_username').val()
        var origin = "<?php echo base_url(); ?>";
        //var origin = "<?php echo base_url(); ?>";
        var siteurl = origin + '/operator/get_waterright';
        var request = $.ajax({
    ...
    function load_client_usage_report() {
        ...
        request.done(function(msg) {
            ...
            data.forEach(element => {
                ...
                Total_Usage += Number(element.meter_vol);
                Remaining += Number(element.remaining);
                Allocation += Number(element.wr_volume);
            });
            rows += '<tr style="border-top: 2px solid #ddd;"><td style="font-weight:bold">Total:</td><td></td><td></td><td></td><td></td><td style="font-weight:bold">' + Total_Usage + ' ML</td><td style="font-weight:bold">' + Remaining + ' ML</td><td style="font-weight:bold">' + Allocation + '</td><td></td></tr>';
    ...
    function load_water_usage_report() {
        ...
        request.done(function(msg) {
            ...
            data.forEach(element => {
                ...
                Total_Usage += Number(element.meter_reading);
                Remaining += Number(element.remaining);
                Allocation += Number(element.wr_volume);
            });

            rows += '<tr style="border-top: 2px solid #ddd;"><td style="font-weight:bold">Total: </td><td></td><td></td><td style="font-weight:bold">' + Total_Usage + ' ML</td><td style="font-weight:bold">' + Remaining + ' ML</td><td style="font-weight:bold">' + Allocation + '</td><td></td></tr>';
    ...
    $(function() {
        $('#edit-details').on('show.bs.modal', function(event) {
            ...
            var ck = button.data('telementry');
            var flow_rate_topic_name = button.data('f_rate_name');
            var flow_rate_scaling = button.data('f_rate_scal');
            var flow_rate_total_name = button.data('f_total_name');
            var flow_rate_total_scaling = button.data('f_total_scal');
            var modal = $(this);
            ...
            if (ck == 1) {
                modal.find('#checkedbox').prop('checked', true);
                $("#chek").show();
            } else {
                modal.find('#checkedbox').prop('checked', false);
                $("#chek").hide();
            }

            modal.find('#edit_flow_rate_topic_name').val(flow_rate_topic_name);

            if (flow_rate_scaling != 0) {
                modal.find('#edit_flow_rate_scaling').val(flow_rate_scaling);
            } else {
                modal.find('#edit_flow_rate_scaling').val();

            }
            modal.find('#edit_flow_total_reading').val(flow_rate_total_name);

            if (flow_rate_total_scaling != 0) {
                modal.find('#edit_flow_total_reading_scaling').val(flow_rate_total_scaling);
            } else {
                modal.find('#edit_flow_total_reading_scaling').val()
            }
    ...
    $("#edit_meter_connection").validate({
        ...
        submitHandler: function(form) {
            ...
            if ($(form).find('input[name="userid"]').val() === '') {
                ...
                var telemetry = $(form).find('select[name="telemetry"]').is(":checked") ? telemetry = 1 : telemetry = 0;
    ...
    // p1 = edit_flow_rate_topic_name
    // p2 = edit_flow_rate_scaling
    // p3 = edit_flow_total_reading
    // p4 = edit_flow_total_reading_scaling


    $("#checkedbox").change(function() {
        if ($("#checkedbox").is(":checked")) {
            if ($('#edit_flow_total_reading').val() == "") {
                $('#edit_flow_rate_topic_name').rules('add', {
                    required: true
                });
            } else {
                $('#edit_flow_rate_topic_name').rules('add', {
    ...
    $("#edit_flow_rate_topic_name").change(function() {
        if ($("#edit_flow_rate_topic_name").val() != "") {
            $('#edit_flow_rate_scaling').rules('add', {
                required: true
            });
        } else {
            $('#edit_flow_total_reading_scaling').rules('add', {
                required: false
            });
            $('#edit_flow_total_reading').rules('add', {
                required: false
            });
        }
        if ($("#edit_flow_rate_scaling").val() != "") {
            $('#edit_flow_rate_topic_name').rules('add', {
                required: true
            });
        }
        if ($("#edit_flow_rate_topic_name").val() == "" && $("#edit_flow_rate_scaling").val() == "") {
            $('#edit_flow_rate_topic_name').rules('add', {
                required: false
            });
            $('#edit_flow_rate_scaling').rules('add', {
        ...
        if ($("#edit_flow_total_reading_scaling").val() != "") {
            $('#edit_flow_total_reading').rules('add', {
                required: true
            });
        }
    ...
    $("#edit_flow_rate_scaling").change(function() {
        if ($("#edit_flow_rate_scaling").val() != "") {
            $('#edit_flow_total_reading').rules('add', {
                required: false
            });
    ...
    $("#edit_flow_total_reading").change(function() {
        if ($("#edit_flow_total_reading").val() != "") {
            $('#edit_flow_total_reading_scaling').rules('add', {
                required: true
            });
        ...
        if ($("#edit_flow_total_reading").val() == "" && $("#edit_flow_total_reading_scaling").val() == "") {
            $('#edit_flow_total_reading').rules('add', {
                required: false
            });
            $('#edit_flow_total_reading_scaling').rules('add', {
    ...
    $("#edit_flow_total_reading_scaling").change(function() {
        if ($("#edit_flow_total_reading_scaling").val() != "") {
            $('#edit_flow_rate_topic_name').rules('add', {
                required: false
            });
        ...
        request.done(function(msg) {
            ...
            const options = {
                createdCell: function(cell, opts) {
                    ...
                    if (opts.row.index == totalRecord - 1) {
                        cell.styles.textColor = "#FFFFFF";
                        cell.styles.setFontType = "bold";
                        cell.styles.fillColor = [40, 126, 183];
                    }
                },
            ...
            var totalRecord = data.rows.length;
    ...
    function generate() {
        ...
        request.done(function(msg) {
            ...
            const options = {
                createdCell: function(cell, opts) {
                    //console.log(opts.row.index);
                    //opts.table.rows.length                    
                    if (opts.row.index == totalRecord - 1) {
                        cell.styles.textColor = "#FFFFFF";
                        cell.styles.setFontType = "bold";
                        cell.styles.fillColor = [40, 126, 183];
    ...
    function load_system_total_report() {
        ...
        request.done(function(msg) {
            ...
            data.forEach(element => {
                ...
                count++;
                sum += Number(element.meter_reading);
                Remaining_sum += Number(element.remaining)
                Allocation_sum += Number(element.wr_volume);
            });
            rows += '<tr style="border-top: 2px solid #ddd;"><td id="total" style="font-weight:bold">Total:</td><td></td><td></td><td></td><td></td><td style="font-weight:bold">' + sum + ' ML</td><td style="font-weight:bold">' + Remaining_sum + ' ML</td><td style="font-weight:bold">' + Allocation_sum + ' ML</td><td></td><td></td><td></td></tr>';
    ...
    $("#order_status,#order_channels,#order_usernames").change(function() {
        ...
        load_water_order_data_archive(order_status, order_channels, order_usernames)
    });


    load_water_order_data(3, 0, 0)

    load_water_order_data_archive(1, 0, 0)
    ...
    function load_water_order_data_archive(order_status, order_channels, order_usernames) {
        let end_point;
        if (order_status == 1) {
            end_point = 'load_allwater_orders_archive';
        } else if (order_status == 2) {
            end_point = 'load_upcoming_water_order';
        } else if (order_status == 3) {
            end_point = 'load_allwater_orders';
        ...
        request.done(function(msg) {
            ...
            $('#water_order_system_archive > tbody').html(rows);
        })
    }
