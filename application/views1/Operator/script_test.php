...
$("#meter_connection_form").validate({
    ...
    submitHandler: function(form) {
        if ($(form).find('input[name="userid"]').val() === '') {
            ...
            // let dataType = "html";
            var origin = window.location.origin;
            var siteurl = origin + '/ICSweb/operator/upload_meter_image';
...
function load_water_usage_report() {
    ...
    var origin = window.location.origin;
    var siteurl = origin + '/ICSweb/operator/load_water_usage_report';
    // var data=data;
    var request = $.ajax({
        url: siteurl,
        method: "POST",
...
var data={show_all:show_all};
    ...
    var origin = window.location.origin;
    var siteurl = origin + '/ICSweb/operator/load_usage_history';
    var request = $.ajax({
        url: siteurl,
        method: "POST",
        data:data,
    ...
    request.done(function(msg) {
        ...
        var count = 1;
        data.forEach(element => {
            rows += '<tr class="display'+ element.active +'"><td><img src="<?php echo base_url(); ?>assets/meterimages/' + element.photo + '" height="50" width="50"></td><td>' + element.serial_number + '</td><td>' + element.meter_name + '</td><td>' + element.meter_reading + '&nbspML</td><td>' + element.date_of_reading + '</td><td></td><td></td><td>' + element.channel_name + '</td><td>' + element.charge_code_value + '</td><td>' + element.type + '</td><td><a href="#" data-toggle="modal" data-keyboard="false" data-backdrop="static" data-target="#edit-meter" data-id="' + element.id + '" data-imagename="' + element.photo + '" data-meter_name="' + element.meter_name + '" data-serial_number="' + element.serial_number + '" data-meter_reading="' + element.meter_reading + '" data-date_of_reading="' + element.date_of_reading + '" data-channel="' + element.channel_id + '" data-charge_code="' + element.charge_code + '" data-type="' + element.type_id + '"> <img src="http://server1.dayopeters.com/ICSweb/assets/operators/img/edit-meter.png" alt="meter"></a></td></tr>';
            count++;
        });
...
$(function() {
    $('#edit-meter').on('show.bs.modal', function(event) {
        ...
        var link = "http://server1.dayopeters.com/ICSweb/assets/meterimages/";
...
$(function() {
    $('#edit-details').on('show.bs.modal', function(event) {
        ...
        var link = "http://server1.dayopeters.com/ICSweb/assets/meterprofiles/";
...
$("#edit_meter_connection").validate({
    ...
    submitHandler: function(form) {
        ...
        if ($(form).find('input[name="userid"]').val() === '') {
            ...
            fd.append('file', files);
            var origin = window.location.origin;
            var siteurl = origin + '/ICSweb/operator/edit_meter_connection';
            fd.append('id', id);
            fd.append('meter_name', meter_name);
            fd.append('channel_name', channel_name);
            fd.append('water_right_number', water_right_number);
