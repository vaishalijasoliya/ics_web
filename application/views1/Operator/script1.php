<script type="text/javascript">
  $(document).ready(function () {
    ...
    $(".editemail").focusout(function(){
      ...
      // console.log("EDIT EMAIL > ", editemail + ' t > ' + typeof editemail)
      if(editemail!=''){
        if( /(.+)@(.+){2,}\.(.+){2,}/.test(editemail) ){
          // valid email
          var origin = window.location.origin;
          var siteurl = origin + '/ICSweb/operator/isEmailExists';
          var request = $.ajax({
            url: siteurl,
            method: "POST",
          ...
          $(".emailvalid").html('* Enter valid email address');
        }
      }
    });
    $("#operatorInputEmail").focusout(function(){
      var origin = window.location.origin;
      var siteurl = origin + '/ICSweb/operator/isEmailExists';
      var isemail = $("#operatorInputEmail").val();
      var request = $.ajax({
        url: siteurl,
        method: "POST",
        data: { email : isemail },
    ...
    // client details populate left boxes on form change
    $('#ud_clientdetail_form select').change(function() {
      ...
      // console.log("INPUT CHANGE > ", username + " contact_name : " + contact_name);
      // if(username===contact_name){
        ...
        request.done(function( msg ) {
         console.log("CLIENT DETAIL : ",msg);
          ...
          // set water rights to id="water_right_number"
      ...
      //   $(".ud_detail_id").val('');
      //   removeWaterRightOptions()
      //   $("#client_selection_response").html('<span class="async_response" style="color:red"> User Name and Contact Name does not matched</span>')
      //   $('.async_response').delay(2500).fadeOut();
      // }
    ...
    function removeWaterRightOptions(){
      $("#water_right_number").html('<option value=""> </option>');
    }
    ...
    // user-details top-left details form on change
    ...
    function load_wateruser_details(){
        ...
        request.done(function( msg ) {
          ...
          users.forEach(element => {
            username += i==0 ? '<option value=""> select</option><option value='+element.id+'>'+element.username+'</option>' : '<option value='+element.id+'>'+element.username+'</option>'
    ...
    function load_meter_connections(){
      ...
      if(response.status!=false){
        ...
        response.forEach(element => {
          rows+='<tr><td>' + element.id + '</td><td>' + element.meter_name +  '</td><td>' + element.channel_name +  '</td><td>' + element.property +  '</td><td>' + element.meter_type +  '</td><td><i class="fa fa-trash"></i></td></tr>'
        });
  ...
  <script>
    $("#search").keyup(function() {
      var value = $(this).val();
      $("#adminuserconfig tbody tr").each(function(index) {
        if (index !== 0) {
          $row = $(this);
          var text = $row.text();
