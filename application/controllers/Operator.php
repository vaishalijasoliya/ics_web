...
class Operator extends CI_Controller
{
    ...
    public function load_live_flow_meters()
    {
        ...
        $allmeter = $this->input->post('allmeter');
        $where = [
            'tbl_meter_connections.telementry' => 1,
            //'tbl_meter_connections.telementry' => 1,
        ];
        if(!empty($userid)){
            $where['tbl_meter_connections.userid'] = $userid;
        }
        if(!empty($channel_id) && $channel_id != 0){
            $where['tbl_meter_connections.channel_name'] = $channel_id;
        }
        ...
        foreach ($details as $value) {
            ...
            $MQTTData = $this->get_live_data_MQTT($value);
        
            if($allmeter == 1){
                $newData = $value;
                $newData['flow_rate'] = $MQTTData['flowRate'];
                $newData['usage'] = $MQTTData['usage'];
                $newData['reading'] = $MQTTData['reading'];
                $newData['latest'] = $MQTTData['latest'];
                array_push($result, $newData);
            } else{ 
                if($MQTTData['flowRate'] > 0){
                    $newData = $value;
                    $newData['flow_rate'] = $MQTTData['flowRate'];
                    $newData['usage'] = $MQTTData['usage'];
                    $newData['reading'] = $MQTTData['reading'];
    ...
    function get_live_data_MQTT($meters){
        $macid = '';
        $flowRate = 0;
        $usage = 0;
        $reading = 0;
